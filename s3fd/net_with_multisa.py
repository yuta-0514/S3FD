...
from s3fd.attention import MultiAttentionNetwork
...
class S3FDNet(nn.Module):
    ...
    def __init__(self, phase='train'):
        ...
        self.MultiAttention256 = MultiAttentionNetwork(channel=256)
        self.MultiAttention512 = MultiAttentionNetwork(channel=512)
        self.MultiAttention1024 = MultiAttentionNetwork(channel=1024)
    ...
    def forward(self, x):
        ...
        for k in range(16):
            x = self.vgg[k](x)
        s = self.MultiAttention(x)
        s = self.L2Norm3_3(x)
        sources.append(s)

        for k in range(16, 23):
            x = self.vgg[k](x)
        s = self.MultiAttention512(x)
        s = self.L2Norm4_3(x)
        sources.append(s)
        ...
        for k, v in enumerate(self.extras):
            ...
            if k == 1:
                x = self.MultiAttention512(x)
                sources.append(x)
            elif k == 3:
                x = self.MultiAttention256(x)
                sources.append(x)
            else :
                continue
