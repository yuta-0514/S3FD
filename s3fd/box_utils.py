...
class Detect(Function):
    ...
    def forward(ctx, loc_data, conf_data, prior_data):
        ...
        conf_preds = conf_data.view(num, num_priors, ctx.num_classes).transpose(2, 1)
        batch_priors = prior_data.view(-1, num_priors, 4).expand(num, num_priors, 4)
        batch_priors = batch_priors.contiguous().view(-1, 4)

        decoded_boxes = decode(loc_data.view(-1, 4), batch_priors, ctx.variance)
        ...
        for i in range(num):
            ...
            for cl in range(1, ctx.num_classes):
                ...
                ids, count = nms(boxes_, scores, ctx.nms_thresh, ctx.nms_top_k)
